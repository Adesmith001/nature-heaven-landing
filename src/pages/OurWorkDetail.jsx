    ...
    <main className="min-h-screen bg-[#F7F0E8] pb-24 pt-32">
      ...
      <article>
        <header className="mx-auto max-w-5xl px-5 py-12 text-center sm:px-8 sm:py-20">
          ...
          <p className="mt-6 text-sm text-[#456456]">By Nurture Haven</p>
        </header>
        {post.coverImageUrl && <div className="mx-auto max-w-7xl px-5 sm:px-8"><img src={post.coverImageUrl} alt={post.coverImageAlt || ''} className="max-h-[700px] w-full rounded-[2rem] bg-white object-contain" /></div>}
        <div className="mx-auto max-w-3xl px-5 py-12 sm:px-8 sm:py-20"><MarkdownContent body={post.body} />
          ...
          {images.length > 0 && <section className="my-14 grid gap-5 sm:grid-cols-2">{images.map((image) => <img key={image.$id} src={image.url} alt={image.altText} className="h-full w-full rounded-3xl bg-white object-contain" loading="lazy" />)}</section>}
        </div>
      </article>
